...
* [`src/cljs/pokeball/`](src/cljs/pokeball/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/pokeball/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/pokeball/`](test/cljs/pokeball/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `pokeball` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/pokeball/config.cljs) defaults to `true` in
...
```clj
(ns pokeball.example
  (:require [pokeball.config :as config])
